...
struct Provider: TimelineProvider {
    func placeholder(in context: Context) -> DailyWisdomEntry {
        DailyWisdomEntry(date: Date(), strategy: dailyBoostStrategies[0])
    }

    func getSnapshot(in context: Context, completion: @escaping (DailyWisdomEntry) -> ()) {
        let entry = DailyWisdomEntry(date: Date(), strategy: getTodaysStrategy())
        completion(entry)
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        ...
        let entry = DailyWisdomEntry(date: currentDate, strategy: getTodaysStrategy())
...
struct DailyWisdomWidget_Previews: PreviewProvider {
    static var previews: some View {
        DailyWisdomWidgetEntryView(entry: DailyWisdomEntry(date: Date(), strategy: dailyBoostStrategies[0]))
